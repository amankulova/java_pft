...
public class DeletionContactFromGroup extends TestBase {
  ...
  public void ensurePreconditions() {
    ...
    if (contacts.size() != 0) {
      ...
      ContactData newContact = new ContactData().withFirstname(RandomStringUtils.randomAlphabetic(10)).withLastname(RandomStringUtils.randomAlphabetic(10)).inGroup(allGroups.stream().iterator().next());
      app.goTo().contactPage();
      app.contact().create(new ContactData().withFirstname("Elton").withLastname("John").withAddress("5 Avenue").inGroup(allGroups.stream().iterator().next()));
